...
class RunSimulationViz:
    ...
    def createFirstPlayer(self):
        ...
        # racecar object
        self.rcs_one = RacecarSimulator(self.car_config)
        ...
        self.mapCallbackOne(map_msg)

        # set ray tracing method
        self.rcs_one.setRaytracingMethod(rospy.get_param("~scan_method"))
    ...
    def createSecondPlayer(self):
        ...
        self.scan_topic = rospy.get_param("~scan_topic_simple")
        self.pose_topic = rospy.get_param("~pose_topic_simple")
        self.odom_topic = rospy.get_param("~odom_topic_simple")
        self.pose_rviz_topic = rospy.get_param("~pose_rviz_topic_simple")
        self.gt_pose_topic = rospy.get_param("~ground_truth_pose_topic_simple")
        ...
        # racecar object
        self.rcs_two = RacecarSimulator(self.car_config)
    ...
    def driveCallbackOne(self, msg):
        ...
        self.rcs_one.drive(msg.drive.speed, msg.drive.steering_angle)


    def poseCallbackOne(self, msg):
        """
        ...
        self.rcs_one.setState(state)
        self.rcs_one.runScan()


    def poseRvizCallbackOne(self, msg):
        """
            same as above
        """
    ...
    def mapCallbackOne(self, map_msg):
        """
        ...
        self.rcs_one.setMap(ros_omap, map_msg.info.resolution, origin)


    def poseTransformPubOne(self, timestamp):
        """
        ...
        if self.broadcast_transform:
            self.br_one.sendTransform(ts)

        if self.pub_gt_pose:
            self.pose_pub_one.publish(ps)


    def steerAngTransformPubOne(self, timestamp):
        """
        ...
        self.br_one.sendTransform(ts_msg)


    def laserLinkTransformPubOne(self, timestamp):
        """
    ...
    def lidarPubOne(self, timestamp):
        """
            Publish lidar
        """

        scan = self.rcs_two.getScan()
    ...
    def driveCallbackTwo(self, msg):
        ...
        self.rcs_two.drive(msg.drive.speed, msg.drive.steering_angle)


    def poseCallbackTwo(self, msg):
        """
            Update current pose on the map
        ...
        self.rcs_two.setState(state)
        self.rcs_two.runScan()


    def poseRvizCallbackTwo(self, msg):
        """
            same as above
        """
        ps_msg = PoseWithCovarianceStamped()
        ps_msg.header = msg.header
        ps_msg.pose = msg.pose.pose
        self.poseCallbackTwo(ps_msg)


    def mapCallbackTwo(self, map_msg):
        """
            Map is changed, pass new map 2d-scanner,
        ...
        # pass map info to racecar instance
        self.rcs_two.setMap(ros_omap, map_msg.info.resolution, origin)


    def poseTransformPubTwo(self, timestamp):
        """
            Publish the transform for pose
        """
        ...
        if self.broadcast_transform:
            self.br_two.sendTransform(ts)

        if self.pub_gt_pose:
            self.pose_pub_two.publish(ps)


    def steerAngTransformPubTwo(self, timestamp):
        """
            Publish steering transforms, left and right steer the same
        """
        ...
        ts_msg.header.frame_id = "front_left_hinge"
        ts_msg.child_frame_id = "front_left_wheel"
        self.br_two.sendTransform(ts_msg)
    ...
    def laserLinkTransformPubTwo(self, timestamp):
        """
            Publish the lidar transform, from base
        """
    ...
    def lidarPubTwo(self, timestamp):
        """
            Publish lidar
        """
